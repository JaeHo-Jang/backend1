...
public class Main1 {
    public static void main(String[] args) {
        ...
        // 존재하는 ID로 검색
        Optional<User> userOptional1 = userRepository.findById(2L); // user.java에서 id를 long으로 선언했으므로 2"L" L을 붙인거임.
        userOptional1.ifPresentOrElse(
                user -> System.out.println("찾는 사용자 : " + user),
                () -> System.out.println("c찾는 사용자가 없습니다.")); // 출력 : 찾는 사용자 : User{id=2, name='Bob'}

        Optional<User> userOptional2 = userRepository.findById(99L);
        userOptional2.ifPresentOrElse(
                user -> System.out.println("찾는 사용자 : " + user),
                () -> System.out.println("c찾는 사용자가 없습니다."));
